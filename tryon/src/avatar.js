...
export function buildAvatar() {
  ...
  const torso = add(new THREE.Mesh(new THREE.LatheGeometry(torsoPts, 56), SKIN));

  // chest definition
  const bustGeo = new THREE.SphereGeometry(0.072, 28, 22);
  [-1, 1].forEach((s) => {
  ...
  // --- Pelvis cap ---
  const pelvis = add(new THREE.Mesh(new THREE.SphereGeometry(0.155, 32, 22), SKIN));
  ...
  const head = add(new THREE.Mesh(new THREE.SphereGeometry(P.headR, 40, 30), SKIN));
  ...
  [-1, 1].forEach((s) => {
    const bang = add(new THREE.Mesh(new THREE.SphereGeometry(0.062, 20, 16), HAIR));
    bang.position.set(s * 0.050, P.headY + 0.050, 0.050);
    bang.scale.set(1.0, 0.72, 0.95);
    bang.rotation.set(-0.10, 0, s * 0.10);
  });
  const backLen = add(new THREE.Mesh(new THREE.CapsuleGeometry(0.115, 0.34, 14, 22), HAIR));
  backLen.position.set(0, P.headY - 0.19, -0.05);
  backLen.scale.set(0.92, 1.0, 0.55);
  [-1, 1].forEach((s) => {
  ...
  function limb(rTop, rBot, len) {
    const geo = new THREE.CapsuleGeometry((rTop + rBot) / 2, len, 14, 22);
    return geo;
  }
  [-1, 1].forEach((s) => {
    ...
    const palm = add(new THREE.Mesh(new THREE.SphereGeometry(0.034, 18, 14), SKIN));
    palm.position.set(s * (P.shoulderX + 0.10), P.shoulderY - 0.555, 0); palm.scale.set(0.82, 1.35, 0.5); palm.rotation.z = s * 0.10;
    const fingers = add(new THREE.Mesh(new THREE.CapsuleGeometry(0.020, 0.05, 10, 14), SKIN));
    fingers.position.set(s * (P.shoulderX + 0.108), P.shoulderY - 0.62, 0); fingers.scale.set(1.15, 1.0, 0.5); fingers.rotation.z = s * 0.10;
  ...
  // --- Legs ---
  [-1, 1].forEach((s) => {
    const thigh = add(new THREE.Mesh(new THREE.CapsuleGeometry(P.thighR, 0.34, 14, 22), SKIN));
    ...
    const calf = add(new THREE.Mesh(new THREE.CapsuleGeometry(P.calfR, 0.32, 14, 22), SKIN));
    calf.position.set(s * P.legX, (P.kneeY + P.ankleY) / 2 + 0.02, 0);

    const foot = add(new THREE.Mesh(new THREE.CapsuleGeometry(0.034, 0.085, 12, 18), SKIN));
    foot.rotation.x = Math.PI / 2;
    foot.position.set(s * P.legX, P.ankleY - 0.028, 0.05);
    const toe = add(new THREE.Mesh(new THREE.SphereGeometry(0.034, 16, 12), SKIN));
    toe.position.set(s * P.legX, P.ankleY - 0.04, 0.115); toe.scale.set(1.0, 0.68, 0.95);
